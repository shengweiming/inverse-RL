...
**RESOLVED (inspection done, n=10 fancy_brackets capped rollouts via
`scripts/inspect_capped.py`):** all ten are degenerate confusion-spirals, not
coherent reasoning truncated mid-thought — `max_completion_len = 512` is
**confirmed**. The model spends 512 tokens because it has no plan, not because it
needs more room (one rollout hallucinated a full English sentence as the "input"
for a 9-char target). Three findings beyond the cap question:

1. **The failure is procedural, not conceptual.** Every rollout describes
   func_15's forward semantics correctly and several identify the correct payload
   characters, then fail the final step — strip the decoration and emit the bare
   string as JSON. One rollout reached the exact correct candidate (`pswqibmjc`)
   and truncated before emitting it; another identified the right answer then
   argued itself out of it ("the question asks for a *different* input"). The
   model is often *close* — the reward-variance condition GRPO needs.
2. **Propose-and-verify (H3) is latent in the base model.** At least one rollout
   named a candidate and gestured at forward-applying func_15 to check it — the
   target strategy flickers but doesn't commit/complete. Mechanistic prior for
   Arm A: RL's job is to make this latent behavior reliable, not invent it.
3. **fancy_brackets confirmed sharpening-prone.** Its 0.53 base coverage is partly
   answer-visibility (payload legible between brackets). Post-RL gains on it are
   the *least* clean evidence for H1 — weight rotate_str/mirror_str (answer
   invisible, base ≈ 0) as load-bearing held-out claims, and lean on the pass@k
   divergence sweep for fancy_brackets specifically (shrinking gap with k ⇒
   reranking, not new capability).
